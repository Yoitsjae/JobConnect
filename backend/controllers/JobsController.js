...
export default class JobsController {
  ...
  static async postNew(req, res) {
    ...
    if (!accessToken) return response.status(401).json({ error: "Unauthorized" });
    ...
    const {
        title, description, budget, deadline, skils
    } = req.body;
    ...
    const newJob = await dbClient.jobsCollection.insertOne({
        description, budget, deadline, clientId: validToken.id, freelancerId: null,
        title, status: "open", createdAt: new Date(), updatedAt: new Date(), skills
    });
  ...
  static async showJob(req, res) {
    ...
    if (req.method = 'PUT') {
      await dbClient.jobsCollection.updateOne(
        {clientId: ObjectID(validToken.id), _id: ObjectID(req.params.id)},
        {$set: {
          status: req.body.status, freelancerId: req.body.freelancerId, updatedAt: new Date()
        }}
      );

      res.status(200).json({id: jobExist._id});
    }
